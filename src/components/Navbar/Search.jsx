...
const Search = () => {
  ...
  return (
    <>
      {/* Search for Medium to Large Screens */}
      ...
      <div ref={searchContainerRef} className="hidden lg:flex relative items-center h-10">
        <div 
          className={`relative flex items-center justify-end transition-all duration-300 ease-in-out rounded bg-[#f3f3f3] ${isSearchOpen ? 'w-48' : 'w-10'}`}
        >
          <input
            ref={inputRef}
            type="search"
            placeholder="Qidiruv"
            className={`h-full pl-10 pr-4 text-sm bg-transparent focus:outline-none text-[#003282] transition-opacity duration-200 ${isSearchOpen ? 'w-full opacity-100' : 'w-0 opacity-0'}`}
          />
          <button
